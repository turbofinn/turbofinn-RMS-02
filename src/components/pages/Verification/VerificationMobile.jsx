    ...
    <React.Fragment>
      ...
      <Box sx={{ bgcolor: "#49C3DE", minHeight: "100vh", display: "flex", flexDirection: "column", alignItems: "center", padding: "0.125rem", fontFamily: "Poppins", position: "relative", overflowY: "auto" }}>
        <Box sx={{ display: "flex", flexDirection: "column", alignItems: "center", minHeight: "100vh", justifyContent: "center", width: "100%", paddingBottom: "2rem" }}>
          <Box sx={{ position: "absolute", top: 0, left: 0, right: 0, backgroundImage: `url(${backgroundfood})`, backgroundSize: "cover", backgroundPosition: "center", height: "50vh", width: "100%", opacity: 0.4 }}></Box>

          <Typography variant="h4" sx={{ color: "white", marginTop: "6.87%", zIndex: 1, fontSize: "2.5rem", fontWeight: 700 }}>LOGO</Typography>
            ...
            {isOtpSent ? (
              <Box sx={{ display: "flex", alignItems: "center", justifyItems: "center", marginLeft: 7 }}>
                <OtpInput value={otp} onChange={setOtp} numInputs={4} renderInput={(props) => (
                  <input {...props} style={{ width: "52px", height: "52px", borderRadius: "11px", border: "2.5px solid transparent", background: "linear-gradient(white, white) padding-box, linear-gradient(90deg, #515ADA 0%, #2B3074 100%) border-box", margin: "0 4px", fontSize: "24px", fontWeight: "bold", textAlign: "center", color: "#000", outline: "none" }}
              ...
              <Input onChange={(event) => { const value = event.target.value.replace(/\D/g, "").slice(0, 10); setMobileNumber(value); event.target.value = value; }}
                inputProps={{ inputMode: "numeric", pattern: "[0-9]*", maxLength: 10 }} startAdornment={<InputAdornment position="start">+91</InputAdornment>} disableUnderline
                sx={{ backgroundColor: "white", borderRadius: "0.625rem", fontSize: "1.3rem", border: "0.09375rem solid rgba(31, 104, 87, 1)", fontWeight: 600, height: "3.4375rem", width: "20.875rem", padding: "0.5rem", marginBottom: "0.5rem", "& .MuiInputBase-root": { height: "100%", alignItems: "center", padding: "0 0.5rem" }, "& .MuiInputBase-input": { padding: 0, "&::placeholder": { color: "black", opacity: 0.5 } }, "&:focus": { borderColor: "black" } }}
            ...
            <Box sx={{ display: "flex", alignItems: "center", marginTop: "-5px" }}>
              {isOtpSent ? (
                <>
                  ...
                  <Button onClick={sentOtpClickHandler} sx={{ color: "rgba(0, 0, 0, 0.8)", fontSize: "13px", fontWeight: 500, margin: 0, textDecoration: "underline", marginLeft: 1, marginTop: 2, padding: 0, minWidth: "auto", textTransform: "none", "&:hover": { backgroundColor: "transparent", textDecoration: "underline" } }}>
                    Resend Code
                  </Button>
                </>
              ) : (
                <>
                  <FormControlLabel control={<Checkbox size="12px" />} sx={{ marginRight: "2px", display: "flex", alignItems: "center", justifyContent: "center", marginLeft: "39px" }}></FormControlLabel>
            ...
            <AnimatePresence mode="wait">
              {isOtpSent ? (
                <motion.div {...anime(dragUpVerifyBtn)} key="verify" style={{ width: "100%", height: "100%" }}>
                  <Button component={Link} variant="contained" sx={{ borderRadius: "21px", backgroundColor: "rgba(9, 146, 176, 0.9)", marginTop: "8.62%", marginBottom: "8.77%", width: "12.8rem", height: "3rem", "&:focus": { bgcolor: "rgba(9, 146, 176, 0.9)" }, boxShadow: "0px 0px 9.5px 0px rgba(0, 0, 0, 0.25)" }} onClick={() => { verifyOTP(mobileNumber, otp); console.log("OTP", otp); }}>
                    <Typography component={motion.div} {...anime(dragDownVerifyTxt)} style={{ fontSize: "0.9375rem", fontWeight: 700, textTransform: "none" }}>
                      {!loader ? (<CircularProgress size="2rem" style={{ color: "white", margin: 'auto', display: 'flex', justifyContent: 'center' }} />) : (<>Verify</>)}
                    </Typography>
                  </Button>
                </motion.div>
              ) : (
                <motion.div {...anime(dragUpSendOtpBtn)} key="SendOtp" style={{ width: "100%", height: "100%" }}>
                  <Button variant="contained" sx={{ height: "3rem", borderRadius: "21px", backgroundColor: "rgba(9, 146, 176, 0.9)", marginTop: "8.62%", marginBottom: "8.77%", width: "12.8rem", "&:focus": { bgcolor: "rgba(9, 146, 176, 0.9)" }, boxShadow: "0px 0px 9.5px 0px rgba(0, 0, 0, 0.25)" }} onClick={sentOtpClickHandler}>
                    <Typography component={motion.div} {...anime(dragDownSendOtpTxt)} style={{ fontSize: "0.9375rem", fontWeight: 700, textTransform: "none" }}>
                      {!loader ? (<CircularProgress size="2rem" style={{ color: "white", margin: 'auto', display: 'flex', justifyContent: 'center' }} />) : (<>Send OTP</>)}
                    </Typography>
                  </Button>
                </motion.div>
              )}
